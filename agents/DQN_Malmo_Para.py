...
def DQN_experiment(params, log_dir, local_log_path, random_seed=None):
    ...
    with Manager() as manager:
        ...
        for trial in range(params["NUM_TRIALS"]):
            ...
            # prepare agents not in parallel
            with Pool(processes=params["NUM_AGENTS"]) as pool:
                ...
                for aid in range(params["NUM_AGENTS"]):
                    ...
                    give = vec_share[abs(0-aid)]
                    receive = vec_share[abs(1-aid)]
                    req_give = vec_request[abs(0-aid)]
                    req_receive = vec_request[abs(1-aid)]

                    p = pool.apply_async(train, [params, log_dir, local_log, RANDOM_SEED, trial, aid, ('1000'+str(aid)),
...
def train(params, log_dir, local_log, random_seed, trial, agent_id, port, 
          give, receive, req_give, req_receive):

    # define device on which to run # repetitive
    device = torch.device(params["DEVICE"])    

    # create agent - moved to a separate method
    print(port)
    agent = prep_agent(params, log_dir, local_log, RANDOM_SEED, trial, agent_id, port)
    ...
    while not agent.completed:
        ...
        if params["SHARING"]:
            ...
            del req_receive[:]
            # then extend it
            req_receive.extend(agent.exp_buffer.identify_unexplored())
